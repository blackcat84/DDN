...
from data.data_loader_one_random_uncert import BSDS_RCFLoader, BIPED_Loader, NYUD_Loader, \
    BSDS_Loader, PASCAL_Loader, COD_Loader
...
parser.add_argument('--kl_weight', default=1e-2, type=float, help='weight for kl norm loss')
parser.add_argument('--sampling', default=500, type=int,
                    help='sampling times in test')
...
parser.add_argument('--encoder', default="efficient-b7", type=str,
                    help='efficient-b7 DDN-M36 VGG')
parser.add_argument('--model', default="model.sigma_logit_unetpp", type=str, help=' ')
parser.add_argument('--mode', default="train", type=str)
parser.add_argument('--note', default="", type=str, help=' ')
...
def cross_entropy_loss_RCF(prediction, labelef, ada, lmba=None):
    with torch.no_grad():
        ...
        mask[mask == 0] = lmba * num_positive / (num_positive + num_negative)
        mask[mask == 2] = 0
...
def main():
    args.cuda = True
        ...
        datadir = "/workspace/00Dataset/LPCB-BSDS"
        train_dataset = BSDS_RCFLoader(root=datadir, split="train", nostd=True)
        test_dataset = BSDS_RCFLoader(root=datadir, split="test", nostd=True)
    ...
    model = Model.Mymodel(args, encoder_name=args.encoder).cuda()
    ...
    if args.mode == "test":
        ...
        test(model, test_loader, "best",
             save_dir=join(TMP_DIR, 'testing-record-view'))
    ...
    for epoch in range(args.start_epoch, args.maxepoch):
        ...
        if args.dataset in ["COD", "SOD"]:
            testdir = os.path.join(args.output, 'eval_results', str(epoch))
            test_cod(test_loader, model, testdir)
        else:
...
def test(model, test_loader, epoch, save_dir):
    ...
    for idx, (image, filename) in enumerate(tqdm(test_loader)):
        ...
        result_mean = (result_mean - result_mean.min()) / (result_mean.max() - result_mean.min())
        ...
        result_std = (result_std.max() - result_std) / (result_std.max() - result_std.min())
